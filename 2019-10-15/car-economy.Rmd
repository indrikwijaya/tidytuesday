...
big_epa_cars %>% 
  filter(cityE == 0) %>% 
  mutate(VClass = fct_reorder(drive, city08)) %>% 
  ggplot(aes(VClass, city08)) + 
  geom_boxplot() + 
  coord_flip()
...
Hypothesis: Engine size started shrinking around 2009 (probably regulatory pressure), especially in large and midsize cars, and this led to