...
peaks %>% 
  ...
  labs(x = "Height (meters)",
       y = "",
       title = "Tallest peaks in the Himalays",
       fill = "")

na_reasons <- c("Unknown", "Attempt rumoured", "Did not attempt climb",
...
- Rate of death over time / per mountain (by all members/hired members)
...
peaks_summarized <- 
  ...
  inner_join(peaks %>% 
               select(peak_id, height_meters), by = "peak_id")
...
peaks_eb %>% 
  ...
  labs(x = "Death rate (empirical Bayes adjusted + 95% credible interval",
       y = "",
       title = "How deadly is each mountain ?",
...
everest_by_decade %>% 
  ...
  geom_point(aes(y = pct_hired_staff_deaths, color = "Hired staff"))+
...
everest_by_decade %>% 
  ...
  labs(x = "Decade",
       y = "Death rate",
       title = 'Everest climbs have been getting more successful over time',
       size = '# of climbers')
```

```{r}
everest_by_decade %>% 
  ggplot(aes(decade, pct_hired_staff_deaths)) +
  geom_line() +
  geom_point(aes(size = hired_staff)) +
  scale_x_continuous(breaks = seq(1970, 2010, 10),
                     labels = c("< 1980", seq(1980, 2010, 10))) +
  scale_y_continuous(labels = percent) + 
  expand_limits(y = 0) +
  labs(x = "Decade",
       y = "Death rate (among hired staff)",
       title = 'Everest has been getting less deadly over time for hired staff, until recently',
       size = '# of hired staff')
```
- Why is the death rate lower from 1980-1990?
...
model <- 
  ...
  glm(died ~ year + age + sex + leader + hired * oxygen_used, data = .,
      family = "binomial")
...
- ML on 21k people ask questions like season, age, sex and the likelihood of them dying
